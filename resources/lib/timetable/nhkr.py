...
class Scraper(Common, Const, PrefData):
    ...
    def parse(self, data):
        ...
        name = data['n1']['following']['area']['name']
        buf1 = self.setup1(data['n1'], 'nhk1', f'NHKラジオ第1({name})')
        buf2 = self.setup1(data['n2'], 'nhk2', f'NHKラジオ第2')
        buf3 = self.setup1(data['n3'], 'nhk3', f'NHK-FM({name})')
        return {
            f'NHKラジオ第1({name})': buf1,
            f'NHKラジオ第2': buf2,
            f'NHK-FM({name})': buf3,
        }

    def setup1(self, data, type_, name):
        return [
            #self.setup2(data['previous'], type_, name),
            self.setup2(data['present'], type_, name),
            self.setup2(data['following'], type_, name),
        ]

    def setup2(self, data, type_, name):
        '''
